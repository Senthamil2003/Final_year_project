...
export default function SpeechModule() {    
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', justifyContent: 'flex-end', fontFamily: 'Arial, sans-serif', backgroundColor: '#f2f2f2', padding: '20px' }}>
        <h1 style={{textAlign:"center",fontFamily: 'Arial, sans-serif'}}>GENAI Interview</h1>
        <div style={{ flex: 1 }}>
          <div style={{ overflowY: 'scroll', maxHeight: '540px', marginBottom: '20px',height:"530px" }}>
            {data.map((item, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: i % 2 !== 0 ? 'flex-start' : 'flex-end', marginBottom: '10px' }}>
                <div style={{ backgroundColor: i % 2 !== 0 ? '#d9e1e8' : '#007bff', color: i % 2 !== 0 ? '#333' : '#fff', padding: '10px', borderRadius: '10px', maxWidth: '70%' }}>
          ...
          <button style={{ margin: '0 5px', padding: '10px 20px', background: '#007bff', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer' }} onClick={SpeechRecognition.startListening}>Start</button>
          <button style={{ margin: '0 5px', padding: '10px 20px', background: '#007bff', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer' }} onClick={SpeechRecognition.stopListening}>Stop</button>
          <input value={write} type='text' style={{ margin: '0 5px', padding: '10px 20px', border: '1px solid #ccc', borderRadius: '5px', flex: 1 }} onKeyPress={handleEnterPress}  onChange={(e)=>setwrite(e.target.value)}></input>
          {!isloading ? <button style={{ margin: '0 5px', padding: '10px 20px', background: '#007bff', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer' }} onClick={submit}>Submit</button>
  :<button style={{ margin: '0 5px', padding: '10px 20px', background: '#007bff', color: '#fff', border: 'none', borderRadius: '5px', cursor: 'pointer' }} > <i class="fa fa-circle-o-notch fa-spin"></i>  Loading</button>}
